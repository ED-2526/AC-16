...
def dense_sift(image, n, step=8, patch_size=16):
    ...
    # debug: which keypoints were removed by SIFT (compare ids)
    inicio3 = {kp_id(kp): kp for kp in keypoints}
    os.makedirs(f"../../debug/4norma/{n}/", exist_ok=True)

    # 5) filtrar por norma del descriptor
    keypoints, descriptors = filtrar_norma(descriptors, keypoints, threshold=0.3)
